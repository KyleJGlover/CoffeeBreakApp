...
//
// Giving thanks to Appacoda for teaching me how to build the rest client and making this so much easier.
// site:  www.appacoda.com
...
class FetchDrinks: ObservableObject {
    @Published var drinks = [Drink]()
    let dm = DataModel()
    
    init() {
        dm.get_drinks_by_id(for: 1) { (drinks) in
        
        DispatchQueue.main.async {
            self.drinks = drinks
            print("*x*: getting drinks.\(drinks[0])")
        }
    }
}
}
